...
export const importFiles: NonNullable<(typeof plugin)['importFiles']> = async ({
  ...
  for (const pathPattern of pathPatterns) {
    for (const locale of settings.locales || []) {
      const path = pathPattern.replace(/{(locale|languageTag)}/, locale);
      filePathMap.set(locale, path);
    }
  }
  ...
  for (const file of files) {
    ...
    // 使用文件名或从映射中获取路径
    const filePath = filePathMap.get(file.locale);
    ...
    const flattened = flatten(parsedContent, { safe: true }) as Record<
      string,
      string
    >;
